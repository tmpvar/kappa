    ...
    proxy: function (req, reply) {
        ...
        reply.proxy({
            ...
            mapUri: function (request, callback) {
                ...
                // HACK: Avoid causing nginx (in the case of registry-static) from
                //       redirecting, which may cause hostname jumps depending on
                //       the deployment. (e.g., docker w/ named network links)
                var finalUrl = options.format()

                if (finalUrl.indexOf('.tgz') === -1 && options.pathname.substring(0, 3) !== '/-/') {
                  finalUrl += '/'
                }

                callback(null, finalUrl, options.headers);
            },
